...
def render_svg(series: PriceSeries, output_path: Path) -> None:
    ...
    price_min = min(min(values) for values in series.prices.values())
    price_max = max(max(values) for values in series.prices.values())
    ...
    def price_y(value: float) -> float:
        normalized = (value - price_min) / price_range
    ...
    price_trendlines = {asset: compute_trendline(values) for asset, values in series.prices.items()}
    ...
    # Draw grid lines based on price axis ticks
    price_ticks = 6
    for i in range(price_ticks):
        value = price_min + (price_range / (price_ticks - 1)) * i
        y = price_y(value)
        svg_parts.append(
            f"<line x1='{margin_left}' y1='{y:.2f}' x2='{margin_left + plot_width}' y2='{y:.2f}' "
            "stroke='#e0e0e0' stroke-dasharray='4 6' />"
        )
        svg_parts.append(_svg_text(margin_left - 10, y + 5, _format_currency(value), anchor="end"))
    ...
    # Plot price series and trendlines
    for asset in ASSETS:
        price_points = list(zip(xs, (price_y(val) for val in series.prices[asset])))
    ...
    svg_parts.append(_svg_text(width / 2, margin_top - 25, "ABTC vs BTC Price Trends & Daily Fluctuations", size=22))
    svg_parts.append(_svg_text(width / 2, height - 25, "Date", size=16))
    svg_parts.append(_svg_text(margin_left - 60, margin_top - 30, "Price", anchor="middle", size=16))
